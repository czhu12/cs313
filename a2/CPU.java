...
public class CPU extends AbstractY86CPU.Sequential {
  ...
  @Override protected void fetch () throws Register.TimingException {
	...
	case 0x0:
	  f.stat.set (S_AOK);
	  break;
	//---- TODO // had to make sure that the 0x4 isnt seen as an error
	case 0x4:
	  if(f.iCd.getValueProduced() == I_MRMOVL || f.iCd.getValueProduced() == I_RMMOVL){
	    f.stat.set(S_AOK);
	  }
	  break;
	case 0x9:
	...
	case I_CALL:
	    if(f.iFn.getValueProduced() == 9){
		f.valC.set(mem.readIntegerUnaligned(F.pc.get() + 2));
		System.out.println(mem.readIntegerUnaligned(F.pc.get() + 2));
	    }else{
  ...
  @Override protected void decode () throws Register.TimingException {
	...
	case I_PUSHL:
	case I_POPL:
	    if(D.iCd.get() == I_CALL && D.iFn.get() == 9){
		d.srcB.set(D.rB.get());
	    }else{
		d.srcB.set (R_ESP);
	    }
  ...
  @Override protected void execute () throws Register.TimingException {
	...
	break;
      case I_CALL:
      case I_PUSHL:
	if(E.iCd.get() == I_CALL && E.iFn.get() == 9){
	    aluA = E.valB.get();
	    System.out.println("aluA is" + aluA);
	}else{
	    aluA = -4;
	}
	break;
	...
	break;
      ...
      case I_POPL:
	if(E.iCd.get() == I_CALL && E.iFn.get() == 9){
	    //this is for the indirect jump
	    aluB = E.valC.get();
	    System.out.println("aluB is " + aluB);
	}else{
	//---- TODO //add the bitshift to valB
		aluB = E.valB.get();
        	if(E.iCd.get() == I_RMMOVL || E.iCd.get() == I_MRMOVL){
        	    if(E.iFn.get() == 0x4){
        		aluB = aluB << 2;
        	    }
        	}
	...
	break;
      case I_OPL:
      //---- TODO set aluFun for iaddl and stuff
      case 0xC:
      //----
	aluFun = E.iFn.get();
	setCC  = true;
  ...
  @Override protected void memory () throws Register.TimingException {
	...
	case I_CALL:
	    if(M.iFn.get() == 9){
		System.out.println("val E is " + M.valE.get());
		System.out.println(mem.readInteger(M.valE.get()));
  ...
  private void newPC () {
	...
	if(E.iFn.get() == 9){
	    w.pc.set(W.valM.get());
	    System.out.println("setting pc to " + W.valM.get());
	}else{
	    w.pc.set (E.valC.get());
	}
